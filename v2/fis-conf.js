...
var fs = require('fs');
var mapFile = fis.get('public_path')+'/map.json';
...
function resetMapFile(path) {
  fs.readFile(path, 'utf8', function (err, data) {
      if (err) {
          throw err;
      }
      fs.writeFile(path, "__RESOURCE_MAP__",function(){
          console.log('reset '+path+' success');
      });
  });
}

deleteFolder(rootFile);
resetMapFile(mapFile);
...
.match('::package', {
  ...
  postpackager: fis.plugin('loader',{
    ...
    allInOne:  {// 默认 false, 配置是否合并零碎资源
      ...
      'js': function(file) {
        console.log(file.filename,'js');
        return fis.get('public_path')+'/'+fis.get('pro_name')+'/js/'+file.filename+'.js';
      },
      'css': function(file) {
        console.log(file.filename,'css');
        return fis.get('public_path')+'/'+fis.get('pro_name')+'/css/'+file.filename+'.css';
      },
      includeAsyncs: true
      // sourceMap: true
    }, 
...
.match('${dev_path}/${pro_name}/(**.css)', {
  release: '${public_path}/${pro_name}/dist/$1',
  // optimizer: fis.plugin('clean-css')
})
